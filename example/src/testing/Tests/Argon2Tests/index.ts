...
// @ts-expect-error
import * as argon2 from 'react-native-quick-argon2';
...
// this actually `node-crypto` tests
// but i've made libs with compatible api
export function registerArgon2Tests() {
    ...
    describe('set options', () => {
      ...
      it('hash with low time cost', async () => {
        try {
          ...
          assert(/invalid timeCost.+between \d+ and \d+/i.test((err as Error).message));
        }
      });
      ...
      it('hash with low hash length', async () => {
        try {
          ...
          assert(/invalid hashLength.+between \d+ and \d+/i.test((err as Error).message));
        }
      });
      ...
      it('hash with low memory cost', async () => {
        try {
          ...
          assert(/invalid memoryCost.+between \d+ and \d+/i.test((err as Error).message));
        }
      });
      ...
      it('hash with low parallelism', async () => {
        try {
          ...
          assert(/invalid parallelism.+between \d+ and \d+/i.test((err as Error).message));
        }
      });
