...
static Options toOptions(Runtime &runtime, const jsi::Object &options) {
    return {options.hasProperty(runtime, "secret")
                ? arrayBufferToVector(
                      runtime, options.getPropertyAsObject(runtime, "secret"))
                : ustring{},
            options.hasProperty(runtime, "associatedData")
                ? arrayBufferToVector(runtime, options.getPropertyAsObject(
                                                   runtime, "associatedData"))
                : ustring{},
            ...
            static_cast<uint32_t>(
                options.getProperty(runtime, "version").asNumber()),
